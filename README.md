...
│   ├── settings.gradle.kts                # Maven 仓库配置（填厂商地址）
...
联系你的机器人厂商，获取适配机器人 OS 版本的 SDK jar 文件，重命名为 `robotservice.jar`，放入：

```
OrionClaw/app/libs/robotservice.jar
```

同时修改 `OrionClaw/settings.gradle.kts`，将 Maven 仓库地址改为厂商提供的实际地址：

```kotlin
// 找到这一行，替换为厂商实际 Maven 地址：
url = uri("https://your-vendor-maven-repo/")
// 如有用户名/密码，同步修改 credentials 部分
```
...
A: 确认 `app/libs/robotservice.jar` 已放置且文件名正确，以及 `settings.gradle.kts` 中的 Maven 仓库地址已更新为厂商实际地址。
